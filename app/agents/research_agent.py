...
research_agent = Agent(
    ...
    model=get_llm(),
    instruction="You are a clinical research assistant. Your task is to analyze patient vitals. Step 1: Look up normal reference ranges for each vital sign using lookup_reference_range. Step 2: Compare patient values to these ranges using analyse_vitals. Step 3: Call classify_health_status to deterministically aggregate status (healthy, at-risk, critical). Step 4: Write a clinical text summary using generate_summary. Step 5: Format the final DiagnosticReport object by calling format_report with the analysis, summary, and record details.",
    tools=[
        lookup_reference_range,
        analyse_vitals,
        classify_health_status,
