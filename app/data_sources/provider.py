    ...
    if settings.DATA_SOURCE.lower() == "bigquery":
        if (
            ...
            if bq_source.health_check():
                ...
                return bq_source
            logger.warning(
                "BigQuery health check failed. Falling back to Mock data source."
            )
        else:
            logger.warning(
                "BigQuery configuration incomplete. Falling back to Mock data source."
            )
